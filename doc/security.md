...
after the pinned records and before the sets: the name, the two endpoints and
the public half of its own key pair are all in the configuration already, and
every operation on a user's own tree begins by resolving her own name, so she
would otherwise have to pin herself in order to name herself.
It comes before the sets because reading a set is itself an Upspin read, which
authenticates, which needs that same key; a server whose own user is not
pinned would go to the network, possibly to itself, to learn a key it is
holding.
